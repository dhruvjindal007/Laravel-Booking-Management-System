...
@section('dashContent')
  <form action="{{Request::segment(2) == 'add' ? route('booking.save') : route('booking.update', ['id' => Request::segment(2)])}}"
    method="POST">
    @csrf
    <div class="container">
    ...
    <div class="mb-3 w-50">
      ...
      <input type="text" class="form-control" id="booking_name" name="booking_name" placeholder="Booking Name"
      value="{{isset($booking->name) ? $booking->name : ''}}">
    </div>
    <div class="mb-3 w-50">
      <label for="booking_on" class="form-label">Booking on</label>
      <input type="date" class="form-control" id="booking_on" name="booking_on" placeholder="Booking Date"
      value="{{isset($booking->booking_datetime) ? $booking->booking_datetime : ''}}">
    </div>
    <div class="mb-3 w-50">
      ...
      <option value="2" {{isset($booking->status) && $booking->status == 2 ? 'selected' : '' }}>Booking cancelled
      </option>
      <option value="3" {{isset($booking->status) && $booking->status == 3 ? 'selected' : '' }}>Booking Fulfilled
      </option>
      </select>
    </div>
