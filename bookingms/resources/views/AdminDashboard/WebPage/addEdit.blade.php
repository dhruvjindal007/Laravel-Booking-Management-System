...
@section('dashContent')
  <form
    action="{{Request::segment(2) == 'add' ? route('webpage.save') : route('webpage.update', ['id' => Request::segment(2)])}}"
    method="POST">
    @csrf
    <div class="container">
    <div class="mb-3 w-50">
      ...
      value="{{isset($data->name) ? $data->slug : ''}}">
    </div>
    <div class="mb-3 w-50">
    ...
    <div class="mb-3 w-50">
      ...
      <textarea id="editor" class="h-100" id="pagecontent" name="page_content"
      rows="40">{{isset($data->html) ? $data->html : ''}}</textarea>
    </div>
    <div class="mb-3 w-50">
